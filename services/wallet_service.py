...
class WalletService:
    ...
    def get_or_create_wallet(self, user: TelegramUser) -> Wallet:
        ...
        wallet, created = Wallet.objects.get_or_create(
            user=user, defaults={"balance": settings.pricing.initial_balance, "currency": "UZS"}
        )
        if created:
    ...
    def get_balance_info(self, user: TelegramUser) -> BalanceInfo:
        ...
        total_credited = transactions.filter(type="credit").aggregate(total=Sum("amount"))[
                             "total"
                         ] or Decimal("0.00")

        total_debited = transactions.filter(type="debit").aggregate(total=Sum("amount"))[
                            "total"
                        ] or Decimal("0.00")
    ...
    def add_balance(
        ...
        try:
            ...
            return TransactionResult(
                success=True, transaction_id=str(transaction.id), balance_after=wallet.balance
            )
        ...
        try:
            ...
            if reference_id:
                existing = Transaction.objects.filter(
                    reference_id=reference_id, user=user, status="completed"
                ).first()

                if existing:
                    logger.warning(
                        f"Transaction with reference_id {reference_id} already exists for user {user.id}"
                    )
                    return TransactionResult(
                        success=False,
    ...
    def get_transaction_history(
            self, user: TelegramUser, limit: int = 50, offset: int = 0
    ) -> List[Transaction]:
    ...
    async def calculate_transcription_cost(self, duration_seconds: int, media_type: str) -> Decimal:
