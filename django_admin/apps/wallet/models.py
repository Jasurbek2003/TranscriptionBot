...
class Wallet(models.Model):
    ...
    balance = models.DecimalField(
        max_digits=12, decimal_places=2, default=Decimal("1000.00"), verbose_name=_("Balance")
    )
    currency = models.CharField(max_length=3, default="UZS", verbose_name=_("Currency"))
    ...
    total_credited = models.DecimalField(
        max_digits=12, decimal_places=2, default=Decimal("0.00"), verbose_name=_("Total Credited")
    )
    total_debited = models.DecimalField(
        max_digits=12, decimal_places=2, default=Decimal("0.00"), verbose_name=_("Total Debited")
    )
    ...
    last_transaction_at = models.DateTimeField(
        null=True, blank=True, verbose_name=_("Last Transaction At")
    )
    ...
    def deduct_balance(self, amount: Decimal, description: str = None):
        ...
        if self.balance < amount:
            raise InsufficientBalanceError(required=float(amount), available=float(self.balance))
    ...
    def get_daily_spent(self):
        ...
        spent = Transaction.objects.filter(
            wallet=self, type="debit", status="completed", created_at__date=today
