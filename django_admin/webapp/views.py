...
from apps.pricing.utils import calculate_transcription_cost, get_active_pricing
...
def home(request):
    ...
    return render(request, "landing.html", {"bot_username": settings.bot_username or "Test_NJ_bot"})
...
def auth_with_token(request):
    ...
    try:
        ...
        logger.debug(f"Total tokens in database: {OneTimeToken.objects.all().count()}")
        token = OneTimeToken.objects.get(token=token_value)
        logger.info(f"Token found: {token.id} for user {token.user_id}")
        # Check if valid
        if not token.is_valid():
            logger.warning(f"Token {token.id} is not valid (expired or already used)")
            return render(
                request,
                "error.html",
                {
...
def dashboard(request):
    ...
    try:
        ...
        recent_transcriptions = Transcription.objects.filter(user=request.user).order_by(
            "-created_at"
        )[:5]
        ...
        return render(
            ...
            "dashboard.html",
            {
                ...
                "settings": {
                    "pricing": pricing,
                    "ai": {"max_file_size_mb": settings.ai.max_file_size_mb},
                },
            },
        )
...
def payment_page(request):
    ...
    try:
        ...
        return render(request, "payment.html", {"user": request.user, "wallet": wallet})
    except Wallet.DoesNotExist:
        return render(
            request,
            "error.html",
...
def telegram_webapp_auth(request):
    ...
    try:
        ...
        if not init_data:
            return JsonResponse(
                create_telegram_auth_response(False, "No initData provided"), status=400
            )
        ...
        if not validated_data:
            return JsonResponse(
                create_telegram_auth_response(False, "Invalid or expired initData"), status=401
            )
        ...
        if not user_data or not user_data.get("telegram_id"):
            return JsonResponse(
                create_telegram_auth_response(False, "Could not extract user data"), status=400
            )
        ...
        return JsonResponse(create_telegram_auth_response(False, "Invalid JSON data"), status=400)
    except Exception as e:
        logger.error(f"Telegram WebApp auth error: {e}", exc_info=True)
        return JsonResponse(
            create_telegram_auth_response(False, "Authentication failed"), status=500
        )
...
def download_transcription(request, transcription_id):
    ...
    try:
        ...
        response = HttpResponse(
            transcription.transcription_text, content_type="text/plain; charset=utf-8"
        )
...
def upload_file(request):
    ...
    try:
        ...
        transcription_service = GeminiTranscriptionService(
            ...
            max_output_tokens=getattr(settings.ai, "gemini_max_output_tokens", 8192),
        )
...
def initiate_payment(request):
    ...
    try:
        ...
        logger.info(
            f"Payment initiated: {gateway}, amount: {amount}, transaction: {transaction.id}"
        )
